/*
 * ProtofyingStdVectorWrapper.cpp
 *
...
public:
	...
	explicit StdVectorWrapper(std::size_t size = 0, T const & value = T() ):
		data( std::vector<T>(size, value) ) {}

	~StdVectorWrapper() {
		std::cerr << "Now destructing an StdVectorWrapper" << std::endl;
	}
...
struct LazyVectorGrammar
  : proto::or_<
        proto::terminal< StdVectorWrapper<_> >
...
template< typename T >
struct lazy_vector
  : lazy_vector_expr< typename proto::terminal< StdVectorWrapper<T> >::type >
{
    typedef typename proto::terminal< StdVectorWrapper<T> >::type expr_type;

    lazy_vector( std::size_t size = 0, T const & value = T() )
      : lazy_vector_expr<expr_type>( expr_type::make( StdVectorWrapper<T>(size, value) ) )
    {}
